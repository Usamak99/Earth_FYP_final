...
const authCtrl = {
  register: async (req, res) => {
    try {
      ...
      const emailRegex = /^[a-zA-Z0-9._%+-]+@(gmail|hotmail|yahoo|icloud)\.com$/;
      const isValidEmail = emailRegex.test(email);
      
      if (!isValidEmail) {
        return res.status(400).json({ msg: "Please enter a valid email address with a supported domain (gmail.com, hotmail.com, yahoo.com, icloud.com)." });
      }


      const passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{6,}$/;
      if (!passwordRegex.test(password)) {
        return res.status(400).json({ msg: "Password must contain at least one lowercase letter, one uppercase letter, one number, one special character, and be at least 6 characters long." });
      }
  ...
  login: async (req, res) => {
    try {
      ...
      setTimeout(() => {
        res.json({msg: ""}); // clears the message after 5 seconds
